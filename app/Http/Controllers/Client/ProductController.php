...
class ProductController extends Controller
{
    ...
    {
        $data['list'] = Product::where(['status'=>1])->orderBy('id','DESC')->select('id','category','name','sku','date_start','description', 'avatar','images','slug','cate_slug','type_slug')
        ->paginate(12);
    ...
    public function allListCate($danhmuc)
    {
        ...
        ->orderBy('id','DESC')
        ->select('id','category','name','sku','date_start', 'avatar','images','slug','cate_slug','type_slug','description')
        ->paginate(12);
    ...
    public function allListType($cate,$type){
        ...
        ->select('id','category','name','sku','date_start', 'avatar','images','slug','cate_slug','type_slug','description')
        ->paginate(12);
        $data['pronew'] = Product::where('status',1)->orderBy('id','DESC')->select('id','category','name','sku','date_start', 'avatar','images','slug','cate_slug','type_slug')
        ->paginate(5);
    ...
    public function detail_product($cate,$slug)
    {
        ...
        ])->where(['slug'=>$slug, 'status'=>1])->first(['id','name','images','type_cate','category','sku','date_start', 'avatar','content','description','slug','cate_slug','type_slug','status']);
        $data['news'] = Blog::where('status',1)->limit(8)->get(['id','title','image','description','created_at','slug']);
        $data['productlq'] = Product::where(['cate_slug'=>$cate, 'status'=>1])->get(['id','name','images','sku','date_start', 'avatar','slug','cate_slug','type_slug','description','category']);
